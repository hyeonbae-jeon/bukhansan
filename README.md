...
## 이번에 바뀐 점 (최신)
1. **형태소 분석기(kiwipiepy) 도입** — 키워드 추출이 이제 정규식 대신 실제 한국어 형태소 분석으로 명사만 정확히 뽑아요. 품질이 훨씬 좋아집니다. (설치가 안 되는 예외 상황이면 자동으로 기존 정규식 방식으로 대체되니 사이트가 깨질 걱정은 없어요.)
2. **통계 대시보드 추가** — 사이트 우측 패널 기본 화면이 이제 "논문을 선택하면 상세정보가 나옵니다" 안내 대신, 총 수집 건수 · 카테고리별 분포 · **연도별 연구 트렌드(어느 해에 어떤 주제의 연구가 많았는지 누적 막대그래프로)** 를 보여줘요. 논문을 클릭하면 상세정보로 바뀌고, "← 전체 통계로 돌아가기"를 누르면 다시 대시보드로 돌아갑니다.
...
- `fetch_naver.py` — 북한산 관련 논문을 네이버 학술정보(academic.naver.com)에서 수집하는 스크립트 (GitHub Actions가 실행, kiwipiepy로 키워드 추출)
